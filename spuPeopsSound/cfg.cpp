...
void ReadConfig(void)
{
	...
	fp = _fsopen( "plugins\\spuPeopsSound.ini", "r", _SH_DENYNO );
	if( !fp )	fp = _fsopen( "spuPeopsSound.ini", "r", _SH_DENYNO );


	if( fp ) {
		fscanf_s( fp, "Version = %s\n", ini_version );
		
		if( strcmp( ini_version, INI_VERSION ) == 0 ) {
			fscanf_s( fp, "SoundDriver = %d\n", &iOutputDriver );
			fscanf_s( fp, "Timer = %d\n", &iUseTimer );
			fscanf_s( fp, "Volume = %d\n", &iVolume );
			fscanf_s( fp, "Interpolation = %d\n", &iUseInterpolation );
			fscanf_s( fp, "Reverb = %d\n", &iUseReverb );
			fscanf_s( fp, "Mono = %d\n", &iDisStereo );
			fscanf_s( fp, "UseXA = %d\n", &iUseXA );
			fscanf_s( fp, "XAPitch = %d\n", &iXAPitch );
			fscanf_s( fp, "IRQWait = %d\n", &iSPUIRQWait );
			fscanf_s( fp, "Debugger = %d\n", &iDebugMode );
			fscanf_s( fp, "Recorder = %d\n", &iRecordMode );
			fscanf_s( fp, "EmuType = %d\n", &iEmuType );
			fscanf_s( fp, "ReverbBoost = %d\n", &iReverbBoost );

			fscanf_s( fp, "Latency = %d\n", &iLatency );
			fscanf_s( fp, "XAFilter = %d\n", &iXAInterp );
			fscanf_s( fp, "CDDAFilter = %d\n", &iCDDAInterp );
			fscanf_s( fp, "Output1Filter = %d\n", &iOutputInterp1 );
			fscanf_s( fp, "Output2Filter = %d\n", &iOutputInterp2 );
			fscanf_s( fp, "XAVol = %d\n", &iVolXA );
			fscanf_s( fp, "CDDAVol = %d\n", &iVolCDDA );
			fscanf_s( fp, "VoiceVol = %d\n", &iVolVoices );
			fscanf_s( fp, "XAStrength = %d\n", &iXAStrength );
			fscanf_s( fp, "CDDAStrength = %d\n", &iCDDAStrength );
			fscanf_s( fp, "Output2Strength = %d\n", &iOutput2Strength );
	...
	if( reset ) {
		...
		if( !fp ) fp = _fsopen( "spuPeopsSound.ini", "w", _SH_DENYNO );
	...
	fp = _fsopen( "plugins\\spuPeopsSound_expert.ini", "r", _SH_DENYNO );
	if( !fp ) fp = _fsopen( "spuPeopsSound_expert.ini", "r", _SH_DENYNO );
	...
	if( fp ) {
		...
		if( strcmp( ini_version,INI_VERSION_EXPERT ) == 0 ) {
			fscanf_s( fp, "LatencyTarget = %d\n", &temp );
			if( temp != -1 ) latency_target = temp;

			fscanf_s( fp, "UploadTimer = %d\n", &upload_timer );
			fscanf_s( fp, "SoundStretcher = %d\n", &sound_stretcher );
			fscanf_s( fp, "\n" );
			
			fscanf_s( fp, "APUcycles = %d\n", &APU_run );
			fscanf_s( fp, "PhantomPad = %d\n", &phantom_padder );
			fscanf_s( fp, "PhantomPadSize = %d\n", &phantom_pad_size );
			fscanf_s( fp, "PhantomPostPad = %d\n", &phantom_post_pad );
			fscanf_s( fp, "LatencyRestart = %d\n", &latency_restart );
			fscanf_s( fp, "UploadLowReset = %d\n", &upload_low_reset );
			fscanf_s( fp, "UploadHighFull = %d\n", &upload_high_full );
			fscanf_s( fp, "UploadHighReset = %d\n", &upload_high_reset );

			fscanf_s( fp, "ReverbTarget = %d\n", &temp );
			if( temp != -1 ) reverb_target = temp;

			fscanf_s( fp, "AsyncWaitBlocker = %d\n", &async_wait_block );
			fscanf_s( fp, "AsyncOndemandBlocker = %d\n", &async_ondemand_block );
			fscanf_s( fp, "DebugSoundBuffer = %d\n", &debug_sound_buffer );
			fscanf_s( fp, "DebugCdxaBuffer = %d\n", &debug_cdxa_buffer );
	...
	if( reset ) {
		// Defaults
		fp = _fsopen( "plugins\\spuPeopsSound_expert.ini", "w", _SH_DENYNO );
		if( !fp ) fp = _fsopen( "spuPeopsSound_expert.ini", "w", _SH_DENYNO );
